...
// `rewriteTargetRelpath` patch in converter-index.ts
// (resources/<DisplayName>[<modid>]/... → config/txloader/forceload/<DisplayName>[<modid>]/...),
// PT 18818 downloads land directly under `config/txloader/forceload/` at the
// exact packaged path, so packing `resources` too would just duplicate them.
