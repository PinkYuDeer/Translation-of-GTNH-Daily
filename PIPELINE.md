...
- 若英文变了而 4964 没跟上，写入 stale 标记：`${新英文}\n旧译：\n${旧译文}`，stage=0；上传到 PT 时保留真实换行。其中 `旧译` payload **优先用我方 18818 的现有译文**，仅当我方无译文时才退用 4964 的旧译（18818 每日跟随 4964，我方漂移行所基于的英文不会比 4964 旧；上游没更新时若取 4964 会把参考退回更旧的译文）
...
│   └── release.yml            手动发版：复用 daily 拉取+打包链（只读 PT）+ NEC 字库
└── ISSUE_TEMPLATE/
    ├── 0-FOS.md
    ├── 1-daily-sync.yml
