...
**(a) 正常 push**：对 `push-queue` 每条 `{pt-path, key, translation, stage}`：
- 从 `file-ids/<pt-path>.strings.json` 取 `stringId`
- `POST /projects/18818/strings/{stringId}/translation`，body `{translation, stage}`
...
- 取 `stringId`，`POST /projects/18818/strings/{stringId}/translation`，body `{translation: marker, stage: 0}`（`stage=0` 以使其进入 PT "未翻译/待审"队列，引人工注意）
