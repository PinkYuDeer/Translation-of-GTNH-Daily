...
- 🔄 **每日汉化项目（自动同步）**：[ParaTranz 项目 18818](https://paratranz.cn/projects/18818) — 本仓库每日从上游拉取最新英文原文，把 4964 的校对译文整合进来后整文件同步
...
```
 ...
 │ ParaTranz 18818      │─当前──┴▶│  （还原换行、打包 7z）   │       ┌─────────────────┐
 └──────────────────────┘          └──────────────────────────┘       │ 每日 7z Release │
                                                                      └─────────────────┘
```

核心思路：**以上游英文原文为主轴**、**4964 校对译文优先于 18818 存量**、**换行符逐词条原样还原**、**最终打包结构对齐线下参考包**。每日中国时间凌晨 1 点自动跑，也可在 Actions 页手动触发（含强制模式）。
...
所有在 PT 上参与汉化的 [贡献者](https://paratranz.cn/projects/4964/members)。
